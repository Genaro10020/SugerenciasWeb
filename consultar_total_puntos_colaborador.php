<?php
    ...
    $total = 0;
            ...
            $consulta = "SELECT * FROM canjer_premios_colaborador_sugerencias WHERE numero_nomina = '$numero_nomina' AND (status='Pte. Solped' OR status='Pte. Entrega' OR status='Entregado')";
            $query = mysqli_query($conexion,$consulta);
            if(mysqli_num_rows($query)>0){
                while ($fila=$query -> fetch_array()){
